...
\lhead{Can Web Technologies Help R Generate Print Quality Graphics?}
...
\vspace{5mm}
\noindent
\textbf{NOTE:} Each of the following examples have been produced with the layoutEngine using a newly developed layoutEngine Backend as part of this research. The code for these examples are found within the Appendix of this report.
...
\item Sophisticated layout and rendering engines within modern browser which are continually being updated to support the latest web standards
\end{itemize}
...
\label{sec:org94b3f60}
\subsubsection{R User Interface}
\label{sec:org0ad8901}
...
\begin{itemize}
\item Default browser opens a tab with a newly supplied random port on every call
\item The API is built with one-to-one wrappers of the DOM scripting methods which creates a tightly coupled connection which might miss some functionality if not purposefully captured
\item The supplied API is incomplete and does not capture all HTML and CSS functionalities
\end{itemize}
...
\label{sec:orgebf3f71}

The RSelenium Backend development effort is captured entirely within the \textbf{\href{https://github.com/kcullimore/layoutenginerselenium}{layoutEngineRSelenium R Package}} \cite{repo03}. It has been developed with the use of two preexisting technologies. The first incorporated technology is the \textbf{\href{https://www.selenium.dev/}{Selenium WebDriver}} \cite{base19} which is a robust browser automation tool. The second is the \textbf{\href{https://cran.r-project.org/web/packages/RSelenium/index.html}{RSelenium R package}} \cite{base24} which provides an interface to the Selenium WebDriver from within R. The Selenium WebDriver is used from supported Docker images which they host on their \textbf{\href{https://hub.docker.com/u/selenium}{Selenium Docker Hub}} site \cite{base25}.
...
\subsection{Solution Design}
\label{sec:org77ae5e8}

\begin{center}
\begin{center}
\includegraphics[width=6in]{/project/research/resources/img/rselenium-backend.png}
\end{center}
\textsf{RSelenium Backend Design}
\end{center}

\subsubsection{Primary Components}
\label{sec:org1a91b74}
\begin{itemize}
\item RSelenium Backend 
\begin{itemize}
\item The RSelenium Backend is comprised of the components outlined in the diagram above. All of these components must be available and configured correctly on the host machine for the Backend to be operational.
\item The R user must have Docker installed on their host machine and one of the Selenium Docker images available to start a container from. The user must also have the layoutEngineRSelenium R package installed in their R session.
\end{itemize}
\item \href{https://github.com/kcullimore/layoutenginerselenium}{layoutEngineRSelenium R Package} \cite{repo03}
\begin{itemize}
\item This R packages was the principal development effort for this Backend design. It contains all of the necessary function calls to run the Selenium Docker container. It then uses the RSelenium R package to communicate with the Selenium server via its API. Once the container is running, a browser is launch from within the container and the Backend can then gain access to the web page DOM via the Selenium API.
\end{itemize}
\item \href{https://cran.r-project.org/web/packages/RSelenium/index.html}{RSelenium R package} \cite{base24}
\begin{itemize}
\item The RSelenium R package provides access to the Selenium API from within R.
\end{itemize}
\item \href{https://hub.docker.com/u/selenium}{Selenium Docker container} \cite{base25}.
\begin{itemize}
\item A Selenium Docker container is needed to run the RSelenium Backend. The container is a fully functional Selenium server and WebDriver combined with a single browser type. The R user can choose to use either a Chromium, FireFox or Opera browser by selecting the correct image.
\end{itemize}
\end{itemize}

\subsubsection{Function Call Relationships}
\label{sec:orgb9733c1}

\begin{enumerate}
\item RSelenium Backend Arguments
\label{sec:org31b6dc8}

\ttfamily   
\begin{itemize}
\item - url='127.0.0.1' /rightarrow local URL to serve the page
\item - portRS=4444 /rightarrow Docker container Selenium server port
\item - portClient=4444 /rightarrow Docker container host machine port
\item - network='bridge' /rightarrow Docker container network type
\item - shm\textunderscore size='1g' /rightarrow Docker container shm memory to allocate
\item - browser\textunderscore type='firefox' /rightarrow Selenium browser type to use \{Chrome, Firefox, Opera\}
\item - headless=FALSE /rightarrow If the browser should be run in headless mode
\item - image\textunderscore request=NULL /rightarrow User supplied Selenium image to pull
\item - fresh\textunderscore pull=FALSE /rightarrow Should Docker pull the latest image version from Docker Hub
\end{itemize}

\normalfont
\item layoutEngineRSelenium Backend Setup
\label{sec:orgbce9b1d}
...
\label{sec:org755e907}
\subsubsection{Benefits}
\label{sec:org0468e87}
...
\label{sec:org213dbb1}

The NodeJS Backend development is captured within two components: (i) the \textbf{\href{https://github.com/kcullimore/layoutengineexpress}{layoutEngineExpress R Package}} \cite{repo04} and (ii) a custom built \textbf{\href{https://hub.docker.com/repository/docker/kcull/layoutengine-express}{NodeJS Backend Server Docker image}} \cite{repo06}. 
...
\item Few risky dependencies (i.e. RSelenium and Selenium images)
...
\subsubsection{Primary Components}
\label{sec:org12f6795}
\begin{itemize}
\item NodeJS Backend 
\begin{itemize}
\item The NodeJS Backend is comprised of the components outlined in the diagram above. All of these components must be available and configured correctly on the host machine for the Backend to be operational.
\item The R user must have Docker installed on their host machine and the NodeJS Backend Server Docker image available to start a container from. The user must also have the layoutEngineExpress R package installed in their R session.
\end{itemize}
\item \href{https://github.com/kcullimore/layoutengineexpress}{layoutEngineExpress R package} \cite{repo04}
\begin{itemize}
\item This R packages was one of the principal development efforts for this Backend design. It contains all of the necessary function calls to run the NodeJS Backend Server Docker container. It then uses a Web Socket connection to communicate with the NodeJS server via a custom API. Once the container is running, the R user must manually launch their browser and navigate to the correct URL of the form \url{http://localhost:port}. Once the browser has established its own client connection to the NodeJS server the bidirectional communication connection is established between the R session and the browser. The Backend can then send web page content to the browser where it will automatically calculate the layout and export the correct data to send back to the R session.
\end{itemize}
\item \href{https://hub.docker.com/repository/docker/kcull/layoutengine-express}{NodeJS Backend Server Docker container} \cite{repo06}
\begin{itemize}
\item The NodeJS Docker container has a custom built \href{https://github.com/kcullimore/express-server}{NodeJS server} \cite{repo05} that has been developed with the use of three preexisting JavaScript projects. The \href{https://webpack.js.org/configuration/dev-server/}{webpack DevServer} \cite{base27} is used as a local server environment that supports live reload of the browser. The \href{https://expressjs.com/}{Express Server} \cite{base28} is used as a lightweight web server that is easy to customize. The \href{https://developer.mozilla.org/en-US/docs/Web/API/WebSockets\_API}{WebSocket API} \cite{base29} is used to support the bidirectional communication protocol between R and the web browser. The Docker container is designed to operate as a server for local hosting of a web page which the layoutEngine can communicate with.
\end{itemize}
\end{itemize}

\subsubsection{Function Call Relationships}
\label{sec:org8f6fa03}

\begin{enumerate}
\item NodeJS Backend Arguments
\label{sec:org9771a80}

\ttfamily   
\begin{itemize}
\item - url='127.0.0.1' /rightarrow local URL to serve the page
\item - portServer=4444, /rightarrow Docker container Selenium server port
\item - portClient=4444 /rightarrow Docker container host machine port
\item - network='bridge' /rightarrow Docker container network type
\item - fresh\textunderscore pull=FALSE /rightarrow Should Docker pull the latest image version from Docker Hub
\end{itemize}
...
\end{enumerate}
\subsection{Solution Evaluation}
\label{sec:org61f12bc}
\subsubsection{Benefits}
\label{sec:orgb9f476d}
...
\begin{itemize}
\item Transformations or formatting can be performed on the server before sending back to R
\item Very tight compatibiity between a Node server and the browser
\end{itemize}
...
\textbf{Limitation} & \textbf{DOM} & \textbf{PhantomJS} & \textbf{CSSBox} & \textbf{RSelenium} & \textbf{NodeJS}\\
\hline
...
\label{sec:orgd87696a}

The \emph{layoutEngine approach} to extending the R graphic system by tapping into an existing rich ecosystem is an intriguing proposition. If the solution design could be developed to behave in a more robust and general purpose manner the payoff would be substantial. The following items would most likely need addressing before a production ready version of the layoutEngine could be built.  
\begin{itemize}
\item layoutEngine:
\begin{itemize}
\item A R user interface that more easily accepts standard R graphical output
\item Support for additional HTML/CSS features to add greater value to the extended functionality
\end{itemize}
\item layoutEngine Backend: 
\begin{itemize}
\item Needs to meets the Minimum Technical Requirements and Target Feature Set covered in this report
\item Needs to be a production ready solution that can handle being used in a wide range of scenarios
\end{itemize}
\end{itemize}
...
\newpage
\section{Appendix}
\label{sec:orge8d70bb}
\sffamily  
\setlength{\parindent}{0em} 
\subsubsection{layoutEngine Setup with RSelenium Backend}
\label{sec:org48cb2eb}
...
\subsubsection{layoutEngine Setup with NodeJS Backend}
\label{sec:orgf6b3230}
...
\newpage 
\subsubsection{Demo 1: Using multiple font types in the same graphical component}
\label{sec:org1438718}
...
\begin{center}
\begin{center}
\includegraphics[width=6in]{/project/research/code/examples/ex-1.png}
\end{center}
\textsf{Example 1: Using Multiple Font Types in the Same Graphical Component}
\end{center}

\newpage
\subsubsection{Demo 2: Show word wrap around transparent PNG}
\label{sec:org390dbad}
...
\begin{center}
\begin{center}
\includegraphics[width=6in]{/project/research/code/examples/ex-2.png}
\end{center}
\textsf{Example 2: Embedded Graphics With Automatically Wrapped Text}
\end{center}

\newpage
\subsubsection{Demo 3: Complex layouts of text and graph components}
\label{sec:org898fcdc}
...
\begin{center}
\begin{center}
\includegraphics[width=6in]{/project/research/code/examples/ex-3.png}
\end{center}
\textsf{Example 3: Complex Layouts Of Text And Graph Components}
\end{center}
