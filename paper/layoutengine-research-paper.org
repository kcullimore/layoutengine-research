...
R users have a powerful tool for generating statistics based graphics but they will struggle to support many of these requirements. Examples include (TODO: provide plot examples): 
  - Using multiple font types in the same graphic (see Appendix XXX)
  - Embedded tables and text-boxes with wrapped text (see Appendix XXX)
  - Complex layouts of text and graph components (see Appendix XXX)
